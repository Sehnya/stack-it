...
const GridCard = ({ post, onRemove }: { post: Post; onRemove: () => void }) => {
  return (
    <motion.div
      ...
      className="bg-white/70 backdrop-blur-sm rounded-2xl border border-gray-200/50 overflow-hidden group hover:shadow-lg transition-all"
    >
        ...
        <div className="relative h-40 overflow-hidden">
          ...
          <div className="absolute top-3 right-3">
            <button
              ...
              className="w-8 h-8 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center text-gray-600 hover:text-red-500 hover:bg-white transition-all opacity-0 group-hover:opacity-100"
            >
              <Trash2 size={14} />
            </button>
          </div>
      ...
      <div className="p-4">
        <NavLink to={`/post/${post.id}`}>
          <h3 className="font-semibold text-gray-900 text-sm line-clamp-2 mb-2 hover:text-gray-700 transition-colors">
            {post.title}
          </h3>
        </NavLink>
        ...
        <div className="flex flex-wrap gap-1 mb-3">
          ...
          {post.technologies.length > 2 && (
            <span className="text-xs text-gray-400">+{post.technologies.length - 2}</span>
          )}
        </div>
...
const ListCard = ({ post, onRemove }: { post: Post; onRemove: () => void }) => {
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: -20 }}
      className="bg-white/70 backdrop-blur-sm rounded-xl border border-gray-200/50 p-4 flex gap-4 group hover:shadow-md transition-all"
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <NavLink to={`/post/${post.id}`}>
              <h3 className="font-semibold text-gray-900 line-clamp-1 hover:text-gray-700 transition-colors">
                {post.title}
              </h3>
            </NavLink>
    ...
    <div className="min-h-screen">
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Saved Stacks</h1>
      ...
      >
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white/70 backdrop-blur-sm rounded-xl border border-gray-200/50 focus:ring-2 focus:ring-gray-300 focus:border-transparent transition-all"
          />
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none pl-9 pr-8 py-2.5 bg-white/70 backdrop-blur-sm rounded-xl border border-gray-200/50 text-sm font-medium text-gray-700 cursor-pointer focus:ring-2 focus:ring-gray-300"
            >
          ...
          {/* View Toggle */}
          <div className="flex items-center bg-white/70 backdrop-blur-sm rounded-xl border border-gray-200/50 p-1">
            <button
      ...
      {loading ? (
        <div className="text-center py-20">
          ...
          <p className="text-gray-500">Loading saved stacks...</p>
        </div>
      ...
      {filteredFavorites.length > 0 && (
        ...
        >
          <div className="flex items-center justify-center gap-8 text-sm text-gray-500">
            <span>
              <strong className="text-gray-900">{filteredFavorites.length}</strong> saved stacks
            </span>
            <span>
