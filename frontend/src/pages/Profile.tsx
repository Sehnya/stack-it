  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="mb-4">
        <NavLink to="/community" className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors">
          <ArrowLeft size={20} />
        ...
        <div className="px-6 pb-6 relative">
          <div className="flex items-end gap-6 -mt-12">
            ...
            <div className="relative">
              <div className="w-24 h-24 rounded-full border-4 border-white bg-gradient-to-br from-pink-200 to-purple-200 p-0.5">
                <img src={avatar} alt={profile.username} className="w-full h-full rounded-full object-cover bg-white" loading="lazy" />
              </div>
            </div>
            ...
            <div className="flex items-center gap-6 pb-2">
              ...
              >
                <span className="text-xl font-bold text-gray-900">{formatCount(profile._count.followers)}</span>
              ...
              <button 
                ...
                className="text-center hover:opacity-70 transition-opacity"
              >
                <span className="text-xl font-bold text-gray-900">{formatCount(profile._count.following)}</span>
              ...
              {posts.map((post, index) => (
                ...
                >
                  <NavLink to={`/post/${post.id}`}>
                    <div className="h-36 overflow-hidden bg-gradient-to-br from-gray-100 to-gray-50">
                      {post.coverImage ? (
                  ...
                  <div className="p-4">
                    <NavLink to={`/post/${post.id}`}>
                      <h3 className="font-semibold text-gray-900 hover:text-gray-700 mb-2 line-clamp-1">{post.title}</h3>
                    </NavLink>
        ...
        {activeTab === 'snippets' && (
          snippets.length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
              ...
              <p className="text-gray-500">No snippets shared yet</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-2">
                          {discussion.tags.slice(0, 4).map(tag => {
                            ...
                            return (
                              <NavLink key={tag} to={`/tech/${encodeURIComponent(tag)}`} className="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600 hover:bg-gray-200">
        ...
        {activeTab === 'followers' && (
          <div className="space-y-4">
            ...
            <div className="flex gap-4 mb-4">
              ...
              <button 
                onClick={() => {}} 
                className="text-sm font-medium text-gray-500 hover:text-gray-700 pb-1"
              >
                ...
                {followers.map((follower, index) => (
                  ...
                  >
                    <NavLink to={`/profile/${follower.id}`} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold text-gray-900 truncate">{follower.username}</h4>
                        {follower.bio && (
