...
const Discussion = () => {
  ...
  if (!discussion) {
    return (
      <div className="text-center py-12">
        ...
        <button onClick={() => navigate('/community')} className="text-blue-600 hover:underline">
          Back to Community
        </button>
      </div>
    )
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-6">
        ...
        {isAuthor && (
          <div className="relative">
            ...
            {showMenu && (
              <div className="absolute right-0 top-full mt-1 bg-white rounded-xl shadow-lg border border-gray-200 py-1 min-w-[160px] z-10">
                <button onClick={handleMarkResolved} className="w-full flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-50">
                  <CheckCircle size={16} /> {discussion.resolved ? 'Mark Unresolved' : 'Mark Resolved'}
                </button>
                <button onClick={() => navigate(`/edit-discussion/${discussionId}`)} className="w-full flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-50">
                  <Edit2 size={16} /> Edit
                </button>
      ...
      {/* Discussion Content */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl border border-gray-200 overflow-hidden mb-6">
        {/* Category & Status */}
        ...
        {/* Title */}
        <h1 className="text-2xl font-bold text-gray-900 px-6 pt-4 pb-2">{discussion.title}</h1>

        {/* Meta */}
        <div className="flex items-center gap-4 px-6 pb-4 text-sm text-gray-500">
          <NavLink to={`/profile/${discussion.author.id}`} className="flex items-center gap-2 hover:text-gray-900">
        ...
        {discussion.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 px-6 pb-4">
            {discussion.tags.map(tag => {
              ...
              return (
                <NavLink key={tag} to={`/tech/${encodeURIComponent(tag)}`} className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-700 hover:bg-gray-200">
        ...
        <div className="px-6 pb-6 border-t border-gray-100 pt-4">
          <div className="prose prose-gray max-w-none" dangerouslySetInnerHTML={{ __html: discussion.content }} />
        </div>
      </motion.div>

      {/* Reply Section */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h3 className="font-semibold text-gray-900">Leave a Reply</h3>
        </div>
      ...
      {discussion.replies > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mt-6 p-6 bg-gray-50 rounded-2xl border border-gray-200 text-center text-gray-500">
